...
<template>
  <div class="min-h-screen bg-gray-50 dark:bg-gray-900 p-6">
    <div class="max-w-4xl mx-auto">
      ...
      <!-- Test Header -->
      <div v-if="test" class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 mb-8">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between">
          <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ test.title }}</h1>
            <p class="text-gray-600 dark:text-gray-400 mt-1">{{ test.description }}</p>
            <div class="flex flex-wrap items-center gap-3 mt-2">
              <p class="text-sm text-gray-500">Total Marks: {{ test.total_marks ?? test.total_points ?? 0 }}</p>
      ...
      <div v-else-if="questions.length === 0" class="text-center py-16 bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700">
        ...
        <p class="text-gray-600 dark:text-gray-400 mb-4">Add your first question to this test</p>
...
const fetchTestDetails = async () => {
  ...
  try {
    // Fetch test details
    const testResponse = await api.get(`/api/admin/tests/${testId}`)
    test.value = testResponse.data
    
    // Fetch questions
    const questionsResponse = await api.get(`/api/admin/tests/${testId}/questions`)
...
const handleQuestionSave = async (questionData) => {
  try {
    if (questionModalMode.value === 'create') {
      await api.post(`/api/admin/tests/${testId}/questions`, questionData)
    } else {
      await api.put(`/api/admin/test-questions/${selectedQuestion.value.id}`, questionData)
    }
...
const deleteQuestion = async () => {
  try {
    await api.delete(`/api/admin/test-questions/${questionToDelete.value.id}`)
    await fetchTestDetails()
    closeDeleteModal()
  } catch (error) {
...
const handleReorder = async () => {
  try {
    ...
    await api.post(`/api/admin/tests/${testId}/reorder-questions`, {
