<template>
    <div class="min-h-screen bg-linear-to-br from-slate-50 via-white to-emerald-50 px-4 py-6 text-gray-900 dark:from-slate-950 dark:via-gray-950 dark:to-emerald-950 dark:text-gray-100 sm:px-6 lg:px-8">
        <div class="mx-auto max-w-7xl space-y-6 lg:space-y-8">
            ...
            <section class="rounded-3xl border border-blue-200 bg-white p-5 shadow-sm dark:border-blue-900 dark:bg-gray-900/80 sm:p-6">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-xs font-semibold uppercase tracking-[0.2em] text-blue-700 dark:text-blue-300">Session Activity</p>
                        <h2 class="mt-1 text-xl font-bold text-gray-900 dark:text-white">Activity Logs</h2>
                    </div>
                    <button
                        @click="fetchActivityData"
                        class="rounded-xl bg-blue-600 px-4 py-2 text-sm font-semibold text-white transition hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-400"
                    >
                        Refresh
                    </button>
                </div>

                <div v-if="activityLoading" class="mt-4 flex items-center justify-center py-8">
                    <div class="h-8 w-8 animate-spin rounded-full border-4 border-blue-600 border-t-transparent"></div>
                </div>

                <div v-else-if="activityError" class="mt-4 rounded-xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-700 dark:border-rose-900 dark:bg-rose-950/40 dark:text-rose-300">
                    {{ activityError }}
                </div>

                <div v-else class="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                    <div class="rounded-2xl border border-blue-200 bg-blue-50/50 p-4 dark:border-blue-900 dark:bg-blue-950/30">
                        <div class="flex items-center justify-between">
                            <p class="text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Status</p>
                            <span class="relative flex h-3 w-3">
                                <span v-if="activity.is_online" class="absolute inline-flex h-full w-full animate-ping rounded-full bg-emerald-400 opacity-75"></span>
                                <span class="relative inline-flex h-3 w-3 rounded-full" :class="activity.is_online ? 'bg-emerald-500' : 'bg-gray-400'"></span>
                            </span>
                        </div>
                        <p class="mt-1.5 text-lg font-bold" :class="activity.is_online ? 'text-emerald-700 dark:text-emerald-400' : 'text-gray-500 dark:text-gray-400'">
                            {{ activity.is_online ? 'Online' : 'Offline' }}
                        </p>
                    </div>

                    <div class="rounded-2xl border border-gray-200 bg-gray-50/50 p-4 dark:border-gray-800 dark:bg-gray-950/50">
                        <p class="text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Session Duration</p>
                        <p class="mt-1.5 text-2xl font-bold text-gray-900 dark:text-white">{{ activity.session_duration || 'N/A' }}</p>
                    </div>

                    <div class="rounded-2xl border border-gray-200 bg-gray-50/50 p-4 dark:border-gray-800 dark:bg-gray-950/50">
                        <p class="text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Last Seen</p>
                        <p class="mt-1.5 text-sm font-semibold text-gray-900 dark:text-white">{{ activity.last_seen_at ? formatDate(activity.last_seen_at) : 'N/A' }}</p>
                    </div>

                    <div class="rounded-2xl border border-gray-200 bg-gray-50/50 p-4 dark:border-gray-800 dark:bg-gray-950/50">
                        <p class="text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Last Login</p>
                        <p class="mt-1.5 text-sm font-semibold text-gray-900 dark:text-white">{{ activity.last_login_at ? formatDate(activity.last_login_at) : 'N/A' }}</p>
                    </div>
                </div>

                <div v-if="!activityLoading && !activityError && activity.ip_address" class="mt-3 text-xs text-gray-500 dark:text-gray-400">
...
const fetchActivityData = async () => {
    ...
    try {
        const { data } = await api.get('/api/my-activity')
        
        activity.is_online = data.is_online || false
        activity.session_duration = data.session_duration || 'N/A'
        activity.last_seen_at = data.last_seen_at || null
        activity.last_login_at = data.last_login_at || null
        activity.ip_address = data.user_ip || 'Unknown'
    } catch (error) {
        activityError.value = error.response?.data?.error || 'Could not load activity data.'
